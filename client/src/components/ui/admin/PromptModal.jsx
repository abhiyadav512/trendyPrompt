...
const PromptModal = ({
    ...
    const handleImageUpload = async (e) => {
        ...
        formDataImg.append("cloud_name", import.meta.env.VITE_CLOUDINARY_CLOUD_NAME);
            ...
            <div className="bg-white dark:bg-gradient-to-br dark:from-gray-900 dark:to-black backdrop-blur-xl border border-gray-300 dark:border-white/20 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto scrollbar-hide transition-colors">
                ...
                {isView ? (
                    <div className="p-6 space-y-4 scrollbar-hide">
                        ...
                        <InfoField label="Prompt Text" value={currentPrompt?.promptText} isBoxed />
                        {currentPrompt?.imgUrl && (
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Image</label>
                                <img src={currentPrompt.imgUrl} alt="Prompt" className="w-full h-48 object-cover rounded-lg" />
                            </div>
                        )}
                    </div>
                ) : (
                    <form onSubmit={handleSubmit} className="p-6 space-y-4 scrollbar-hide">
                        <InputField
                            label="Title"
                            name="title"
                        ...
                        <div>
                            ...
                            {uploading && (
                                <div className="text-sm text-blue-600 mt-2 animate-pulse">Uploading image...</div>
                            )}
                        ...
                        <div className="flex items-center gap-4 pt-4">
                            <button
                                type="submit"
                                disabled={uploading}
                                className={`flex items-center gap-2 px-6 py-3 rounded-lg font-semibold transition-all duration-300 ${uploading
...
const InfoField = ({ label, value, isBoxed = false }) => (
    <div>
        ...
        <p className={`${isBoxed ? 'bg-gray-100 dark:bg-white/5 p-4 rounded-lg' : ''} text-black dark:text-white`}>
            {value}
        </p>
    </div>
);
